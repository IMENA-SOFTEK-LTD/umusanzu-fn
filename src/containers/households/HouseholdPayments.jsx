...
const HouseholdPayments = ({ household }) => {
  ...
  const [
    getPaymentDetails,
    { data: paymentDetailsData, isLoading: paymentDetailsIsLoading, isError: paymentDetailsIsError, isSuccess: paymentDetailsIsSuccess },
    ...
    //  console.log(original)
      if ([5, 3, 1].includes(levelId)) {
        return (
          <span className="flex gap-2">
            <Button
              ...
              className={`${status !== 'PENDING' && levelId !== 5 && 'hidden'} ${
                ['PARTIAL', 'INITIATED', 'PENDING'].includes(status) &&
                levelId === 5 &&
                'flex'
              } ${status === 'PAID' || ['CASH'].includes(paymentMethod) ? 'hidden' : ''} !text-white !bg-red-500 !p-2 !rounded-full hover:!bg-red-600`}
              onClick={(e) => {
                e.preventDefault()
            ...
            <Button
              ...
              className={`${levelId !== 5 && 'hidden'} !p-2 !rounded-full !bg-primary !text-white hover:!bg-primary/80`}
              onClick={(e) => {
                e.preventDefault()
                dispatch(setPayment(original))
        ...
        return (
          <Button
            value="Pay"
            className={`!w-fit ${['PAID'].includes(status) || ['CASH'].includes(paymentMethod) ? 'hidden' : ''}`}
            onClick={(e) => {
              e.preventDefault()
    ...
    () => [
        ...
        Cell: ({ row }) => {
          ...
          const color =
            status === 'PAID'
              ...
              : 'bg-red-600'
          return (
            <span className={`${color} w-auto px-2 py-1 text-white text-xs rounded`}>
              {status}
            </span>
          )
        },
      ...
      { Header: 'Date', accessor: 'date' },
      {
        ...
        Cell: ({ row }) => {
          ...
          return <span className={`${pm === 'Mobile_Money' ? 'text-gray-600' : 'text-blue-600'}`}>{pm === 'Mobile_Money' ? 'MOMO' : pm}</span>
        },
      },
      {
        Header: 'Receipt',
        ...
        Cell: ({ row }) => {
          ...
          return (
            <Button
              value={status === 'PAID' || status === 'INITIATED' ? 'Receipt' : 'Invoice'}
              className={`${status === 'PAID' || status === 'INITIATED' ? '!bg-green-600' : status === 'PENDING' ? '!bg-red-600' : 'bg-yellow-600'} uppercase !w-fit`}
              onClick={(e) => {
                e.preventDefault()
  ...
  const data = useMemo(() => {
    return household?.payments
      ...
      ?.sort((a, b) => {
        const statusOrder = (status) => (status === 'PENDING' ? 0 : status === 'PARTIAL' ? 1 : 2)
  ...
  const MobileRows = useMemo(() => {
    return (
      <div className="flex flex-col w-full gap-2 md:hidden">
        {data?.map((row, idx) => (
          <div key={idx} className="bg-white shadow rounded p-3 flex flex-col gap-2">
            ...
            {expandedRow === idx && (
              <div className="text-sm flex flex-col gap-1">
                ...
                <div>
                  <Button
                    value={row.status === 'PAID' || row.status === 'INITIATED' ? 'Receipt' : 'Invoice'}
